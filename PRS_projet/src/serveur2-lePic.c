...
#define RECVSIZE 1500

int main(int argc, char *argv[]){
    //Verification of pamameters
    //Execute in terminal ./server1-LePic NumeroPort
    if(argc<2){
    ...
    //Creat Socket for Server's Controler
    ...
    //Creat Server' Controler Address
              ...
              while(endFile ==0){
                ...
                switch(select(maxfdp,&fds,NULL,NULL,&timeout)){
                  ...
                  case 0:
                    ...
                    gettimeofday(&starttime,NULL);
                    while((wOccupiedSize<window)&&((len = fread(buffer, sizeof(char), RECVSIZE-6, fp))!=0)){
                      lenEnd = len;
                      memset(sendmsg, 0, RECVSIZE);
                      sprintf(sendmsg,"%.6d",seqSend);
                      memcpy(sendmsg+6, buffer, len);
